...
class PatchLoss(nn.Module):
    ...
    def compute_prototypes(self, emds, mask):
        """
        计算每个掩码的原型。

        Args:
            emds: 特征投影，形状为 [2, 256, 128, 128]。
            mask: 掩码，形状为 [2, N, H, W]。

        Returns:
            prototypes: 每个掩码的原型，形状为 [2, N, 256]。
        """
        batch_size, num_masks, h, w = mask.shape
        _, feat_dim, feat_h, feat_w = emds.shape

        # 调整 mask 和 emds 形状
        emds_flat = emds.view(batch_size, feat_dim, -1)  # [2, 256, 128*128]
        mask_flat = mask.view(batch_size, num_masks, -1)  # [2, N, 128*128]

        # 计算原型，每个掩码区域的平均特征值
        prototypes = torch.einsum("bnm,bfm->bnf", mask_flat, emds_flat)  # [2, N, 256]
        mask_sum = mask_flat.sum(dim=-1, keepdim=True) + 1e-6  # 防止除以 0
        prototypes = prototypes / mask_sum  # 每个掩码的平均特征

        return prototypes

    def forward(self, mask, emds):
        """
        计算 PatchLoss。
        ...
        """
        # Step 1: 计算每个掩码的原型
        prototypes = self.compute_prototypes(emds, mask)  # [2, N, 256]

        # Step 2: 计算相似度矩阵
        batch_size, num_masks, feat_dim = prototypes.shape
        prototypes_flat = prototypes.view(batch_size * num_masks, feat_dim)  # [2*N, 256]
        sim_matrix = torch.mm(prototypes_flat, prototypes_flat.t())  # [2*N, 2*N]

        # Step 3: 构造标签
        labels = torch.arange(batch_size * num_masks).cuda()  # [0, 1, ..., 2*N-1]
...
class HRDAHead(BaseDecodeHead):
    ...
    def forward_train(self,
        ...
        if stage == 1:
            ...
            # 定义 PatchLoss
            patch_loss_fn = PatchLoss(temperature=0.1)

            # 计算损失
            loss_contrast = patch_loss_fn(mask, embedding)
            # print("Patch Loss:", loss.item())
            losses['loss_seg'] = losses['loss_seg'] + self.loss_weight * loss_contrast
        self.reset_crop()
        return losses
